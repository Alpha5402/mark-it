...
export class DocumentController {
  ...
  /**
   * 将原始文本中的 rawOffset 转换为新 model 中的语义偏移
   * 
   * 原理：遍历新 model 的 inline 段，累加每个段的标记符和文本长度，
   * 找到 rawOffset 落在哪个段的哪个位置（标记符内还是文本内），
   * 然后计算对应的语义偏移。
   * 
   * @param blockId block ID
   * @param rawOffset 在原始文本中的偏移（包含结构性标记符和 inline 标记符）
   * @returns 语义偏移（与 computeSemanticOffset 返回值一致），或 null
   */
  rawOffsetToSemanticOffset(blockId: string, rawOffset: number): number | null {
    const block = this.blocks.get(blockId)
    if (!block) return null
    ...
    // rawOffset 在结构性标记符内
    if (rawOffset <= prefixOffset) {
      return rawOffset  // 结构性标记符内的位置直接映射
    }

    // 跳过结构性标记符
    let remaining = rawOffset - prefixOffset
    let semanticOffset = prefixOffset

    if (!block.inline) return prefixOffset

    for (const inline of block.inline) {
      if (inline.type !== 'text') continue

      const prefixLen = (inline.markers && inline.marks !== 0) ? inline.markers.prefix.length : 0
      const suffixLen = (inline.markers && inline.marks !== 0) ? inline.markers.suffix.length : 0
      ...
      if (remaining <= textLen) {
        return semanticOffset + remaining
      }
      remaining -= textLen
      semanticOffset += textLen
  ...
  /**
   * 将原始文本中的 rawOffset 转换为新 model 的 DOM rawOffset
   * 
   * 由于 inlineParse 重新解析后每个 inline 段有独立的 prefix/suffix，
   * DOM 的文本布局（getRawText）可能与输入的原始文本不同。
   * 这个方法将输入原始文本中的 offset 映射到 getRawText 重建文本中的 offset。
   */
  mapRawOffsetToModelRawOffset(blockId: string, inputRawOffset: number): number {
    const block = this.blocks.get(blockId)
    if (!block) return inputRawOffset

    const prefixOffset = this.prefixOffset(blockId)
    
    // 在结构性标记符内，直接返回
    if (inputRawOffset <= prefixOffset) {
      return inputRawOffset
    }

    // 跳过结构性标记符
    let remaining = inputRawOffset - prefixOffset
    let modelOffset = prefixOffset

    if (!block.inline) return modelOffset
    ...
    for (const inline of block.inline) {
      ...
      // 在 prefix 标记符内
      if (remaining <= prefixLen) {
        // 映射到 model 的 prefix 内的对应位置
        return modelOffset + remaining
      }
      remaining -= prefixLen
      modelOffset += prefixLen
