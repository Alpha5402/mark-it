...
describe('parseLine', () => {
  ...
  test('parses task list markers and checked state', () => {
    ...
    expect(unchecked.style).toEqual({ ordered: false, bullet: '-', task: true, checked: false, checkedMarker: undefined, markerSpacing: ' ' })
    expect(checked.style).toEqual({ ordered: false, bullet: '-', task: true, checked: true, checkedMarker: 'x', markerSpacing: ' ' })
  })
